...
class Joblist extends CI_Controller {
	...
	function index($slug)
	{
		if(isset($slug) && $slug!=""){
			...
			$total_row = $this->Index_model->get_job_galleryCount($slug);
			...
			$data['joblist']		= $this->Index_model->get_job_gallery($slug,$config["per_page"],$page);
			//$this->Index_model->getDataById('jobpost','job_category',$slug,'job_id','desc','');
			
			$data['main_content']="frontend/job_list";
