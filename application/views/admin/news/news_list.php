...
function approve(){
	...
	if(data=="")
	{
		...
		return false;
			...
			var hrefdata ="<?php echo base_url('admin');?>/approve?approve_val="+data+"&tablename=news"+"&id=news_id"+"&status=status";
...
function deapprove(){
	...
	if(data=="")
	{
		...
		return false;
			...
			var hrefdata ="<?php echo base_url('admin');?>/deapprove?approve_val="+data+"&tablename=news"+"&id=news_id"+"&status=status";
			window.location.href=hrefdata;
	}
...
function deletedata(tablename){
	...
	if(data=="")
	{
		...
		var b = window.confirm('Are you sure, you want to delete this ?');
		if(b==true){
			var hrefdata ='<?php echo base_url('admin');?>/deleteAllData/'+tablename+'/news_id/'+data;
			window.location.href=hrefdata;
			}
			else{
			 return;
					...
					<div class="breadcrumb-line breadcrumb-line-component" style="margin-top:10px; margin-bottom:10px;">
						...
						<ul class="breadcrumb-elements">
							<div class="heading-btn-group">
								<a href="<?php echo base_url('admin/news_registration');?>" class="btn btn-link btn-float has-text">
				...
				<div class="content">
					...
					<div class="panel panel-flat">
						...
						<table class="table datatable-show-all" width="100%">
                                    ...
                                    <?php
									$i=0;
                                    foreach($news_list->result() as $newsData):
									$news_id=$newsData->news_id;
									$news_title=$newsData->news_title;
									$date=$newsData->date_time;
									$i++;
									?>
                                      <tr>
                                       <td><?php echo $i;?></td>
										...
										<ul class="icons-list">
											<li class="dropdown">
												...
												<ul class="dropdown-menu dropdown-menu-right">
													<li><a href="<?php echo base_url('admin/news_registration/'.$news_id);?>" style="color:#EDBB0E">
                                                    <i class="glyphicon glyphicon-edit"></i> Edit Information</a></li>
													<li><a href="javascript:void();" onclick="openPage1('<?php echo $news_id;?>','news','news_id');" style="color:#ff0000">
