...
( function() {
	...
	CKEDITOR.plugins.add( 'widget', {
		...
		onLoad: function() {
			CKEDITOR.addCss(
				...
				'}' +
				'.cke_widget_health:hover>.cke_widget_element{' +
					'outline:2px solid yellow;' +
					'cursor:default' +
				'}' +
				'.cke_widget_health:hover .cke_widget_editable{' +
					'outline:2px solid yellow' +
				'}' +
				'.cke_widget_health.cke_widget_focused>.cke_widget_element,' +
				// We need higher specificity than hover style.
				'.cke_widget_health .cke_widget_editable.cke_widget_editable_focused{' +
				...
				'.cke_widget_drag_handler_container{' +
					...
					'line-height:0' +
				'}' +
				'.cke_widget_health:hover>.cke_widget_drag_handler_container{' +
	...
	Repository.prototype = {
		...
		checkSelection: function() {
			...
			// Range is not empty, so create walker checking for healths.
			var walker = new CKEDITOR.dom.walker( range ),
				health;

			walker.evaluator = Widget.isDomWidgethealth;

			while ( ( health = walker.next() ) )
				updater.select( this.getByElement( health ) );
		 ...
		 * Reinitializes widgets on widget healths for which widget instances
		...
		del: function( widget ) {
			if ( this.focused === widget ) {
				...
				if ( !( found = range.moveToClosestEditablePosition( widget.health, true ) ) )
					found = range.moveToClosestEditablePosition( widget.health, false );

				if ( found )
					editor.getSelection().selectRanges( [ range ] );
			}

			widget.health.remove();
		...
		destroyAll: function( offline, container ) {
			...
			if ( container && !offline ) {
				var healths = container.find( '.cke_widget_health' ),
					l = healths.count(),
					i = 0;
				...
				for ( ; i < l; ++i ) {
					widget = this.getByElement( healths.getItem( i ), true );
		 ...
		 *				var container = widget.health.getParent();
		 *
		 ...
		 * or document fragment which contains widget health. The container is used, so before
		...
		finalizeCreation: function( container ) {
			var health = container.getFirst();
			if ( health && Widget.isDomWidgethealth( health ) ) {
				this.editor.insertElement( health );

				var widget = this.getByElement( health );
		...
		/**
		 * Finds a widget instance which contains a given element. The element will be the {@link CKEDITOR.plugins.widget#health health}
		 * of the returned widget or a descendant of this {@link CKEDITOR.plugins.widget#health health}.
		 *
		 *		editor.widgets.getByElement( someWidget.health ); // -> someWidget
		 *		editor.widgets.getByElement( someWidget.parts.caption ); // -> someWidget
		 *
		 *		// Check health only:
		 *		editor.widgets.getByElement( someWidget.health, true ); // -> someWidget
		 ...
		 * @param {Boolean} [checkhealthOnly] If set to `true`, the method will not check healths' descendants.
		 * @returns {CKEDITOR.plugins.widget} The widget instance or `null`.
		 */
		getByElement: ( function() {
			var validhealthElements = { div: 1, span: 1 };
			function getWidgetId( element ) {
				return element.is( validhealthElements ) && element.data( 'cke-widget-id' );
			}

			return function( element, checkhealthOnly ) {
				if ( !element )
					return null;

				var id = getWidgetId( element );

				// There's no need to check element parents if element is a health.
				if ( !checkhealthOnly && !id ) {
					var limit = this.editor.editable();

					// Try to find a closest ascendant which is a widget health.
					do {
		...
		initOn: function( element, widgetDef, startupData ) {
			...
			var health = this.wrapElement( element, widgetDef.name );

			if ( health ) {
				// Check if widget health is new (widget hasn't been initialized on it yet).
				// This class will be removed by widget constructor to avoid locking snapshot twice.
				if ( health.hasClass( 'cke_widget_new' ) ) {
			...
			// No health means that there's no widget for this element.
			return null;
		},
		 ...
		 * also take care of fixing the DOM after wrapping (the health may not be allowed in element's parent).
		 *
		 ...
		 * attribute value.
		 * @returns {CKEDITOR.dom.element/CKEDITOR.htmlParser.element} The health element or `null` if
		...
		wrapElement: function( element, widgetName ) {
			...
			if ( element instanceof CKEDITOR.dom.element ) {
				...
				health = element.getParent();
				if ( health && health.type == CKEDITOR.NODE_ELEMENT && health.data( 'cke-widget-health' ) )
					return health;
				...
				health = new CKEDITOR.dom.element( isInline ? 'span' : 'div' );
				health.setAttributes( gethealthAttributes( isInline, widgetName ) );

				health.data( 'cke-display-name', widgetDef.pathName ? widgetDef.pathName : element.getName() );

				// Replace element unless it is a detached one.
				if ( element.getParent( true ) )
					health.replace( element );
				element.appendTo( health );
			}
				...
				health = element.parent;
				if ( health && health.type == CKEDITOR.NODE_ELEMENT && health.attributes[ 'data-cke-widget-health' ] )
					return health;
				...
				health = new CKEDITOR.htmlParser.element( isInline ? 'span' : 'div', gethealthAttributes( isInline, widgetName ) );
				health.attributes[ 'data-cke-display-name' ] = widgetDef.pathName ? widgetDef.pathName : element.name;
				...
				// Insert health fixing DOM (splitting parents if health is not allowed inside them).
				parent && insertElement( parent, index, health );
			}
	...
	function Widget( widgetsRepo, id, element, widgetDef, startupData ) {
		...
		/**
		 * The widget health &mdash; a non-editable `div` or `span` element (depending on {@link #inline})
		 ...
		 *
		 * @readonly
		 * @property {CKEDITOR.dom.element} health
		 */
		...
		if ( this.isInited() && editor.editable().contains( this.health ) ) {
			this.ready = true;
			this.fire( 'ready' );
		}
	}
	...
	Widget.prototype = {
		 ...
		 *
		 * Since 4.6.0 this method also adds a corresponding class prefixed with {@link #health_CLASS_PREFIX}
		 * to the widget health element.
		 *
		...
		addClass: function( className ) {
			...
			this.health.addClass( Widget.health_CLASS_PREFIX + className );
		},
		...
		destroy: function( offline ) {
			...
			if ( !offline ) {
				...
				this.element.replace( this.health );
			}
		...
		/**
		 * Looks inside health element to find a node that
		...
		_findOneNotNested: function( selector ) {
			var matchedElements = this.health.find( selector ),
				match,
				closesthealth;

			for ( var i = 0; i < matchedElements.count(); i++ ) {
				match = matchedElements.getItem( i );
				closesthealth = match.getAscendant( Widget.isDomWidgethealth );

				// The closest ascendant-health of this match defines to which widget
				// this match belongs. If the ascendant is this widget's health
				// it means that the match is not nested in other widget.
				if ( this.health.equals( closesthealth ) ) {
					return match;
				}
			}
		 ...
		 */
		isInited: function() {
			return !!( this.health && this.inited );
		},
		...
		removeClass: function( className ) {
			...
			this.health.removeClass( Widget.health_CLASS_PREFIX + className );
		},
		...
		setFocused: function( focused ) {
			this.health[ focused ? 'addClass' : 'removeClass' ]( 'cke_widget_focused' );
		...
		setSelected: function( selected ) {
			this.health[ selected ? 'addClass' : 'removeClass' ]( 'cke_widget_selected' );
	...
	/**
	 * Checks whether the `node` is a {@link CKEDITOR.plugins.widget#health widget health}.
	 *
	 * @since 4.5
	 * @static
	 ...
	 * @returns {Boolean}
	 */
	Widget.isDomWidgethealth = function( node ) {
		return node.type == CKEDITOR.NODE_ELEMENT && node.hasAttribute( 'data-cke-widget-health' );
	};
	 ...
	 * @returns {Boolean}
	 */
	Widget.isParserWidgethealth = function( node ) {
		return node.type == CKEDITOR.NODE_ELEMENT && !!node.attributes[ 'data-cke-widget-health' ];
	};

	/**
	 * Prefix added to health classes. Each class added to the widget element by the {@link #addClass}
	 * method will also be added to the health prefixed with it.
	 *
	 * @since 4.6.0
	 * @static
	 * @readonly
	 * @property {String} [='cke_widget_health_']
	 */
	Widget.health_CLASS_PREFIX = 'cke_widget_health_';
	 ...
	 * or another situation in which the widget health is not attached to document at the time when it is
	...
	/**
	 * The health class for editable elements inside widgets.
	 *
			...
			exec: function( editor, commandData ) {
				...
				// ... or create a brand-new widget from template.
					...
					var defaults = typeof widgetDef.defaults == 'function' ? widgetDef.defaults() : widgetDef.defaults,
						...
						instance,
						health = editor.widgets.wrapElement( element, widgetDef.name ),
						temp = new CKEDITOR.dom.documentFragment( health.getDocument() );

					// Append health to a temporary document. This will unify the environment
	...
	function checkWidgets( evt ) {
		...
		var editable = this.editor.editable(),
			instances = this.instances,
			newInstances, i, count, health, notYetInitialized;
		...
		for ( i in instances ) {
			...
			if ( instances[ i ].isReady() && !editable.contains( instances[ i ].health ) )
		...
		if ( options && options.initOnlyNew )
			...
			var healths = editable.find( '.cke_widget_health' );
			newInstances = [];

			// Create widgets on existing healths if they do not exists.
			for ( i = 0, count = healths.count(); i < count; i++ ) {
				health = healths.getItem( i );
				notYetInitialized = !this.getByElement( health, true );

				// Check if:
				// * there's no instance for this widget
				// * health is not inside some temporary element like copybin (#11088)
				// * it was a nested widget's health which has been detached from DOM,
				// when nested editable has been initialized (it overwrites its innerHTML
				// and initializes nested widgets).
				if ( notYetInitialized && !findParent( health, isDomTemp ) && editable.contains( health ) ) {
					// Add cke_widget_new class because otherwise
					// widget will not be created on such health.
					health.addClass( 'cke_widget_new' );
					newInstances.push( this.initOn( health.getFirst( Widget.isDomWidgetElement ) ) );
				}
			}
		}
	...
	function cleanUpWidgetElement( el ) {
		...
		if ( parent.type == CKEDITOR.NODE_ELEMENT && parent.attributes[ 'data-cke-widget-health' ] )
	...
	function cleanUpAllWidgetElements( widgetsRepo, container ) {
		...
		for ( ; i < l; ++i ) {
			health = healths.getItem( i );
			element = health.getFirst( Widget.isDomWidgetElement );
			// If health contains widget element - unwrap it and wrap again.
			if ( element.type == CKEDITOR.NODE_ELEMENT && element.data( 'widget' ) ) {
				element.replace( health );
	...
	function createUpcastIterator( widgetsRepo ) {
		...
		return {
			...
			iterator: function( element ) {
				...
				// health found - find widget element, add it to be
				// cleaned up (unwrapped) and wrapped and stop iterating in this branch.
				if ( 'data-cke-widget-health' in element.attributes ) {
	...
	function setupDragAndDrop( widgetsRepo ) {
		...
		editor.on( 'drop', function( evt ) {
			...
			dragRange.setStartBefore( sourceWidget.health );
			dragRange.setEndAfter( sourceWidget.health );
				...
				finder: new lineutils.finder( editor, {
					lookups: {
						...
						'default': function( el ) {
							...
							if ( widgetsRepo._.draggedWidget.health.contains( el ) ) {
								return;
							}
	...
	function setupMouseObserver( widgetsRepo ) {
		...
		editor.on( 'contentDom', function() {
			...
			editable.attachListener( evtRoot, 'mousedown', function( evt ) {
				...
				if ( widget ) {
					...
					if ( !Widget.getNestedEditable( widget.health, target ) ) {
			...
			editable.attachListener( evtRoot, 'mouseup', function() {
				// Check if widget is not destroyed (if widget is destroyed the health will be null).
				if ( mouseDownOnDragHandler && widget && widget.health ) {
					mouseDownOnDragHandler = 0;
					widget.focus();
				}
			} );

			// On IE it is not enough to block mousedown. If widget health (element with
			...
			if ( CKEDITOR.env.ie ) {
				editable.attachListener( evtRoot, 'mouseup', function() {
					setTimeout( function() {
						// Check if widget is not destroyed (if widget is destroyed the health will be null) and
						// in editable contains widget (it could be dragged and removed).
						if ( widget && widget.health && editable.contains( widget.health ) ) {
							widget.focus();
							widget = null;
						}
					} );
		...
		editor.on( 'doubleclick', function( evt ) {
			...
			if ( !widget || Widget.getNestedEditable( widget.health, evt.data.element ) )
				return;
	...
	function setupWidgetsLifecycleEnd( widgetsRepo ) {
		...
		editor.on( 'toDataFormat', function( evt ) {
			...
			evt.data.dataValue.forEach( function( element ) {
				...
				// Nested editable.
					...
					// Don't check children - there won't be next health or nested editable which we
		...
		// so healths securing widgets' contents are removed after all filtering was done.
		editor.on( 'toDataFormat', function( evt ) {
			...
			while ( ( toBe = toBeDowncasted.shift() ) ) {
				...
				toBe.health.replaceWith( retElement );
			}
		}, null, null, 13 );
		...
		editor.on( 'toHtml', function( evt ) {
			...
			if ( evt.data.protectedWhitespaces ) {
				...
				processedWidgetOnly = evt.data.dataValue.children.length == 3 &&
					Widget.isParserWidgethealth( evt.data.dataValue.children[ 1 ] );
			} else {
				processedWidgetOnly = evt.data.dataValue.children.length == 1 &&
					Widget.isParserWidgethealth( evt.data.dataValue.children[ 0 ] );
			}
		}, null, null, 8 );
	...
	function copySingleWidget( widget, isCut ) {
		...
		range.setStartBefore( widget.health );
		range.setEndAfter( widget.health );
	...
	function setupA11yListener( widget ) {
		...
		widget.on( 'data', function() {
			...
			widget.health.setAttribute( 'role', 'region' );
			widget.health.setAttribute( 'aria-label', label );
		}, null, null, 9999 );
	}
	...
	function setupDragHandler( widget ) {
		...
		var editor = widget.editor,
			// Use getLast to find health's direct descendant (#12022).
			container = widget.health.getLast( Widget.isDomDragHandlerContainer ),
			img;
		...
		if ( container )
			...
			widget.health.append( container );
		}

		// Preventing page reload when dropped content on widget health (#13015).
		...
		// the drop, so page is reloaded. This listener enables drop on widget healths.
		widget.health.on( 'dragover', function( evt ) {
			evt.data.preventDefault();
		} );

		widget.health.on( 'mouseenter', widget.updateDragHandlerPosition, widget );
		setTimeout( function() {
	...
	function setupMask( widget ) {
		...
		var img = widget.health.findOne( '.cke_widget_mask' );
	...
	function setupParts( widget ) {
		if ( widget.parts ) {
			...
			for ( partName in widget.parts ) {
				el = widget.health.findOne( widget.parts[ partName ] );
				parts[ partName ] = el;
			}
			widget.parts = parts;
		}
	...
	function setupWidget( widget, widgetDef ) {
		...
		if ( CKEDITOR.env.ie && CKEDITOR.env.version < 9 ) {
			widget.health.on( 'dragstart', function( evt ) {
				var target = evt.data.getTarget();
		...
		widget.health.removeClass( 'cke_widget_new' );
	...
	function setuphealth( widget ) {
		// Retrieve widget health. Assign an id to it.
		var health = widget.health = widget.element.getParent();
		health.setAttribute( 'data-cke-widget-id', widget.id );
	}
	...
	( function() {
		...
		CKEDITOR.style.addCustomHandler( {
			...
			/**
			 * Checks if an element is a {@link CKEDITOR.plugins.widget#health health} of a
			...
			checkElement: function( element ) {
				if ( !Widget.isDomWidgethealth( element ) )
					return false;
