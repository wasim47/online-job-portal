...
( function() {
	...
	function initResizeHandles( dialog ) {
		...
		var healthWidth, healthHeight, viewSize, origin, startSize, dialogCover;
		...
		var mouseDownFn = CKEDITOR.tools.addFunction( function( $event ) {
			...
			healthHeight = startSize.height - dialog.parts.contents.getSize( 'height', !( CKEDITOR.env.gecko || CKEDITOR.env.ie && CKEDITOR.env.quirks ) );
			healthWidth = startSize.width - dialog.parts.contents.getSize( 'width', 1 );
		...
		function mouseMoveHandler( evt ) {
			...
			if ( ( resizable == CKEDITOR.DIALOG_RESIZE_WIDTH || resizable == CKEDITOR.DIALOG_RESIZE_BOTH ) )
				width = Math.max( def.minWidth || 0, internalWidth - healthWidth );

			if ( resizable == CKEDITOR.DIALOG_RESIZE_HEIGHT || resizable == CKEDITOR.DIALOG_RESIZE_BOTH )
				height = Math.max( def.minHeight || 0, internalHeight - healthHeight );
