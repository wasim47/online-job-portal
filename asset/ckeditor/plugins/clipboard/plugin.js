		...
		init: function( editor ) {
			...
			editor.on( 'paste', function( evt ) {
				...
				if ( data.match( /^<[^<]+cke_(editable|contents)/i ) ) {
					var tmp,
						editable_health,
						health = new CKEDITOR.dom.element( 'div' );

					health.setHtml( data );
					// Verify for sure and check for nested editor UI parts. (#9675)
					while ( health.getChildCount() == 1 &&
							( tmp = health.getFirst() ) &&
							tmp.type == CKEDITOR.NODE_ELEMENT &&	// Make sure first-child is element.
							( tmp.hasClass( 'cke_editable' ) || tmp.hasClass( 'cke_contents' ) ) ) {
						health = editable_health = tmp;
					}

					// If editable health was found strip it and bogus <br> (added on FF).
					if ( editable_health )
						data = editable_health.getHtml().replace( /<br>$/i, '' );
				}
	...
	function initPasteClipboard( editor ) {
		...
		function getClipboardDataByPastebin( evt, callback ) {
			...
			setTimeout( function() {
				...
				// We need to look for a apple style health on webkit it also adds
				// a div health if you copy/paste the body of the editor.
