...
( function() {
	...
	function addUploadWidget( editor, name, def ) {
		...
		CKEDITOR.tools.extend( def, {
			...
			init: function() {
				var widget = this,
					id = this.health.findOne( '[data-cke-upload-id]' ).data( 'cke-upload-id' ),
				...
				loader.on( 'update', function( evt ) {
					...
					if ( !widget.health || !widget.health.getParent() ) {
					...
					// Set style to the health if it still exists.
					newStyle = 'cke_upload_' + loader.status;
					if ( widget.health && newStyle != oldStyle ) {
						oldStyle && widget.health.removeClass( oldStyle );
						widget.health.addClass( newStyle );
						oldStyle = newStyle;
					}
			...
			replaceWith: function( data, mode ) {
				...
				range.setStartBefore( this.health );
				range.setEndAfter( this.health );
