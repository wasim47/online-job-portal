...
(function($){
	...
	$.fn.dcDrilldown = function(options) {
		...
		return this.each(function(options){
			...
			var $health = '<div class="'+defaults.classhealth+'" />';
			$($dcDrilldownObj).wrap($health);
			var $dchealth = $($dcDrilldownObj).parent();
			var objIndex = $($dchealth).index('.'+defaults.classhealth);
			var idHeader = defaults.classHeader+'-'+objIndex;
			var idhealth = defaults.classhealth+'-'+objIndex;
			$($dchealth).attr('id',idhealth);
			var $header = '<div id="'+idHeader+'" class="'+defaults.classHeader+'"></div>';

			setUpDrilldown();

			if(defaults.saveState == true){
				var cookieId = defaults.classhealth+'-'+objIndex;
				checkCookie(cookieId, $dcDrilldownObj);
			}

			resetDrilldown($dcDrilldownObj, $dchealth);
			...
			$('li a',$dcDrilldownObj).click(function(e){
				...
				if($('> ul',$activeLi).length){
					if($($link).hasClass(defaults.classActive)){
						...
						actionDrillDown($activeLi, $dchealth, $dcDrilldownObj);
					}
				}
			...
			function setUpDrilldown(){
				...
				// Add css class
				$('ul',$dchealth).each(function(){
					$('li:last',this).addClass('last');
				});
				$('> ul > li:last',$dchealth).addClass('last');
				if(defaults.linkType == 'link'){
					$(objUl).css('top',itemHeight+'px');
				}

			    // Add 'active' class to parent list item in all levels
			    $('ul',$dchealth).find('li.active').parents('li').addClass('active');
			}
			...
			$('#'+idHeader).on('click', 'a', function(e){
				...
				if($(this).hasClass('link-back')){
					linkIndex = $('#'+idhealth+' .'+defaults.classParentLink+'.active-ul').length;
		...
		// Drill Down
		function actionDrillDown(element, health, obj){
			// Declare header
			var $header = $('.'+defaults.classHeader, health);

			// Get new breadcrumb and header text
			var getNewBreadcrumb = $(defaults.headerTag,$header).html();
			var getNewHeaderText = $('> a',element).html();
			//var getParentText = $(health).find('.'+defaults.classMenu).data('category');
			...
			if(defaults.saveState == true){
				var cookieId = $(health).attr('id');
				createCookie(cookieId, obj);
			}
		}

		// Drill Up
		function actionDrillUp(element, obj, health){
			// Declare header
			var $header = $('.'+defaults.classHeader, health);

			var activeLink = $('> a',element);
			var checklength = $('.'+defaults.classActive, health).length;
			var activeIndex = $(activeLink).index('.'+defaults.classActive, health);
		...
		function resetDrilldown(obj, health){
			var $header = $('.'+defaults.classHeader, health);
			$('ul',$header).remove();
			$('a',$header).remove();
			$('li',obj).show();
			...
			$('a.'+defaults.classActive,obj).each(function(i){
				...
				actionDrillDown($activeLi, health, obj);
			});
		}
